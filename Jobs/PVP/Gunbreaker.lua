...
function Gunbreaker:Rush(actions, menu)
	if player:hasStatus(3052) then
	 return true 
	elseif actions.relentless:canUse() and self:EnemiesAround(player, 5) >= menu["RELENTLESS_MIN"].int then
...
function Gunbreaker:Tick(getTarget)
	
	local menu    = self.menu["ACTIONS"]["TANK"]["GNB"]
	local actions = self.actions

	if self:Rush(actions, menu) then return end
